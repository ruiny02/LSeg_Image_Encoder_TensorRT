...
class TRTEncoder:
    ...
    def __init__(self, engine_path: str, input_shape: Tuple[int, int, int, int]):
        ...
        # Set shape once (fixed in this project).
        ...
        self._out = torch.empty(self.out_shape, device="cuda", dtype=torch.float32)
...
def compute_mask_from_features(
    ...
    # Ensure float32 for stable matmul
    ...
    img_flat = image_features.view(c, -1)  # (C, Hf*Wf)
    sim = torch.matmul(text_features, img_flat)  # (N, Hf*Wf)
    sim = sim.view(text_features.shape[0], hf, wf)
    mask = torch.argmax(sim, dim=0)  # (Hf, Wf)
    return mask
...
def main() -> None:
    ...
    while True:
        ...
        # Postprocess: similarity + argmax (GPU) -> mask (CPU)
        mask = compute_mask_from_features(feat, text_features)
        ...
        if not args.no_display:
            # upsample mask to camera resolution
            mask_up = cv2.resize(mask_cpu, (args.cam_w, args.cam_h), interpolation=cv2.INTER_NEAREST)
            color_mask = palette[mask_up]  # (H,W,3) BGR
